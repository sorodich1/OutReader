...
namespace OutReader.Helper
{
    public class DbHelper
    {
        ...
        public static void SetKns(Kns kns)
        {
            ...
            {
                if (kns.MB16Ds.Count > 0 || kns.OBEHs.Count > 0)
                {
        ...
        private static string MB8AsQuery(DateTime lastUpdate, string knsId, List<MB8A> mb8As)
        {
            ...
            if (mb8As != null && mb8As.Count > 0)
            {
                ...
                if (knsId == "KNS06")
                {
                    query += getQueryAnalogKNS(lastUpdate, knsId, 21, mb8As[0].A6); //Моментальный расход 1
                    query += getQueryAnalogKNS(lastUpdate, knsId, 23, mb8As[0].A7); //Моментальный расход 2
                }
            }
        ...
        private static string TERsQuery(DateTime lastUpdate, string knsId, List<TER> ters)
        {
            ...
            {
                query += getQueryAnalogKNS(lastUpdate, knsId, 20, ters[0].V); //Накопленный расход 1
                query += getQueryAnalogKNS(lastUpdate, knsId, 21, ters[0].Q); //Моментальный расход 1
                if (ters.Count > 1)
                {
                    query += getQueryAnalogKNS(lastUpdate, knsId, 22, ters[1].V); //Накопленный расход 1
                    query += getQueryAnalogKNS(lastUpdate, knsId, 23, ters[1].Q); //Моментальный расход 1
                }
            }
